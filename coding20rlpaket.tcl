...
# discount_factor (gamma): seberapa besar reward 
set epsilon 0.1
...
# prosedur myRand yg menghasilkan angka acak dalam rentang [min, max]
proc myRand { min max } {
...
# prosedur myRandfloat yg menghasilkan angka acak float dalam rentang [min, max]
...
# level energi node: tinggi, sedang, rendah
set energy_levels {"high" "medium" "low"}
# level jumlah tetangga: banyak, sedikit
set neighbor_levels {"many" "few"}
# Loop untuk setiap node (i) dari 0 sampai jumlah node - 1
for {set i 0} {$i < $val(nn)} {incr i} {
    # Untuk setiap mode node (misal: sleep, listen, transmit)
    foreach mode $actions {
        # Untuk setiap level energi (high, medium, low)
        foreach el $energy_levels {
            foreach nl $neighbor_levels {
                # Membentuk nama state gabungan dari mode, level energi, dan level tetangga
                set state "$mode-$el-$nl"
                # Untuk setiap aksi yang mungkin diambil node
                foreach action $actions {
                    # Inisialisasi nilai Q-table untuk kombinasi node, state, dan aksi ke 0.0
...
# melakukan inisialisasi status semua node ke sleep
for {set i 0} {$i < $val(nn)} {incr i} {
    set status($i) "sleep"
}
...
proc PowerDissipation {} {
    ...
    for {set i 1} {$i < $val(nn)} {incr i} {
        # memeriksa status node dan kurangi energi sesuai mode
        if {$status($i) == "sleep"} {
        ...
        # memastikan energi tidak menjadi negatif
        if {$energy($i) < 0} {
            set energy($i) 0
        }

        # menandai node dengan status saat ini
        $ns at [$ns now] "$n($i) label $status($i)"
    }

    # menjadwalkan ulang PowerDissipation untuk siklus berikutnya
...
proc ReceivingPower {l node} {
    ...
    # memeriksa apakah node sedang dalam mode sleep
    if {$status($node) == "sleep"} {
...
#Grid Partition
# pembagian grid untuk node ke dalam tiga bagian/ area berbeda
...
proc choose_action {node current_state} {
    ...
    if {[expr rand()] < $epsilon} {
        # Pilih aksi random dari daftar actions
        return [lindex $actions [expr int(rand()*3)]]
    } else {
        # Jika tidak, memilih aksi dengan nilai Q terbesar (eksploitasi)
        set max_q -99999           ;# Inisialisasi nilai Q maksimum
        set best_action ""         ;# Inisialisasi aksi terbaik
        foreach action $actions {
            set q $Q($node,$current_state,$action) ;# mengambil nilai Q untuk aksi ini
            if {$q > $max_q} {
                set max_q $q           ;# Update nilai Q maksimum
                set best_action $action ;# menyimpan aksi terbaik
            }
        }
        return $best_action         ;# mengembalikan aksi dengan Q terbesar
    }
}
...
proc update_transmit_subslot {node selected reward} {
    ...
    for {set j 0} {$j < $m} {incr j} {
        if {$Qslot($node,$j) > $max_q} {
            ...
            # menyimpan nilai Q terbesar
            set max_idx $j
            # menyimpan indeks sub-slot dengan Q terbesar
        }
    }
...
proc update_qvalue {node current_state action reward next_state} {
    ...
    foreach next_action $actions {
        ...
        # mengambil Q-value untuk setiap aksi di next_state
        if {$q > $max_q_next} {
            set max_q_next $q
            # menyimpan nilai Q terbesar
        }
...
# Prosedur utama untuk menjalankan Q-Learning pada semua node secara periodik
proc QLearningNodeMode {} {
    global ns val status actions energy n Q learning_rate prev_energy transmit_success

    # Reset flag keberhasilan transmit untuk semua node sebelum loop
    for {set i 0} {$i < $val(nn)} {incr i} {
        set transmit_success($i) 0
    }

    # Loop untuk setiap node
    for {set i 0} {$i < $val(nn)} {incr i} {
        set current_state [get_state $i]           ;# mengambil state saat ini untuk node i
        set action [choose_action $i $current_state] ;# memilih aksi berdasarkan Q-Learning (epsilon-greedy)
        set prev_energy($i) $energy($i)            ;# menyimpan energi sebelum aksi

        if {$action == "sleep"} {
            sleepMode $i                           ;# Jika aksi sleep, panggil sleepMode
        } elseif {$action == "listen"} {
            listenMode $i 64                       ;# Jika aksi listen, panggil listenMode (paket 64 byte)
        } elseif {$action == "transmit"} {
            activeMode $i 512                      ;# Jika aksi transmit, panggil activeMode (paket 512 byte)
            set selected_slot [choose_subslot $i]  ;# memilih sub-slot untuk transmit
            update_transmit_subslot $i $selected_slot 0 ;# Update Q-slot dan distribusi probabilitas sub-slot
            # Pengiriman paket terjadi lewat attach-cbr-traffic dan callback mark_transmit_success
       } elseif {$action == "transmit"} {
    if {$status($i) == "sleep"} {
        puts "Node $i sedang dalam sleep mode, tidak akan kirim paket"
    } else {
        activeMode $i 512
        set selected_slot [choose_subslot $i]
        update_transmit_subslot $i $selected_slot 0

        # Kirim ke sink hanya jika bukan sink node
        if {$i != $center} {
            set traffic_agent [new Agent/LossMonitor]
            $ns attach-agent $n($center) $traffic_agent
            set cbr [attach-cbr-traffic $n($i) $traffic_agent $center 512 0.05]
            $ns at [$ns now] "$cbr start"
            $ns at [expr [$ns now] + 2.5] "$cbr stop"
        }
    }
}

        set deltaE [expr {$prev_energy($i) - $energy($i)}] ;# menghitung energi yang digunakan
        set beta 1.0                                      ;# Bobot reward keberhasilan transmit
        set S $transmit_success($i)                       ;# Status keberhasilan transmit
        set reward [expr {-1.0 * $deltaE + $beta * $S}]   ;# menghitung reward Q-Learning
        set next_state [get_state $i]                     ;# mengambil state berikutnya setelah aksi
        update_qvalue $i $current_state $action $reward $next_state ;# Update Q-table
        set k 10
        set learning_rate [expr {$learning_rate * (1.0 / (1 + $k))}] ;# Update learning rate (decay)
        set status($i) $action    ;# Update status node ke aksi yang dipilih
    }
    # menjadwalkan pemanggilan ulang prosedur ini setiap 1 detik simulasi
...
for {set des 0} {$des<$val(nn)} {incr des} {
		for {set j 0} {$j<$val(nn)} {incr j} {
# memastikan bahwa tujuan (des) bukan sumber yang sama (j)
			if {$des!=$j} {
...
proc NormalizePolicy {pi} {
    set m [llength $pi]
    # menghitung jumlah elemen dalam list pi (jumlah aksi)
    set d 1.0
    ...
    foreach val $pi {
        if {$val < $d} {
            set d $val
            # mencari nilai terkecil dalam distribusi pi
        }
    }
    if {$d < $delta} {
        set rho [expr $c0 / ($c0 - $delta)]
        # menghitung faktor penyesuaian jika ada nilai di bawah delta
        for {set k 0} {$k < $m} {incr k} {
            set old [lindex $pi $k]
            # mengambil nilai lama pada indeks k
            set newval [expr $c0 - $rho * ($c0 - $old)]
            # menghitung nilai baru dengan penyesuaian
            lset pi $k $newval
    ...
    # menghitung total seluruh nilai distribusi
    for {set k 0} {$k < $m} {incr k} {
        lset pi $k [expr [lindex $pi $k] / $r]
        # Normalisasi setiap nilai agar total menjadi 1
    }
    return $pi
    # mengambalikan distribusi pi yang sudah dinormalisasi
}
...
proc finish {} {
   ...
   # menjalankan NAM untuk memvisualisasikan simulasi
