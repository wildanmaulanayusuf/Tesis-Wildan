...
#successrate RL
set srfile [open RL_SuccessRate.tr w]
...
# Array untuk menyimpan waktu node mati
array set dead_time {}
# Variabel status apakah sudah dicatat FND/HND/LND
set fnd_captured 0
set hnd_captured 0
set lnd_captured 0
# File output otomatis untuk lifetime
set ltf_auto [open Lifetime_Auto.tr w]
# Fungsi untuk update status node mati
proc check_node_death {node} {
    global energy ns n dead_time val fnd_captured hnd_captured lnd_captured ltf_auto
    # Jika node belum pernah tercatat mati dan energi <= 0
    if {![info exists dead_time($node)] && $energy($node) <= 0} {
        set dead_time($node) [$ns now]
        puts $ltf_auto "Node $node died at [$ns now] sec"
        # Hitung jumlah node mati
        set dead_nodes [array size dead_time]
        # FIRST NODE DEAD
        if {$fnd_captured == 0} {
            puts $ltf_auto "First node died at [$ns now]"
            set fnd_captured 1
        }
        # HALF NODE DEAD
        if {$hnd_captured == 0 && $dead_nodes >= [expr int($val(nn)/2)]} {
            puts $ltf_auto "Half nodes died at [$ns now]"
            set hnd_captured 1
        }
        # LAST NODE DEAD
        if {$lnd_captured == 0 && $dead_nodes == $val(nn)} {
            puts $ltf_auto "All nodes died (network lifetime ends) at [$ns now]"
            set lnd_captured 1
        }
    }
}
...
#RL Success Rate
array set total_transmit {}
array set success_transmit {}
...
proc mark_transmit_success {src dst} {
    global transmit_success ns success_transmit transmitted_this_round
    # Pastikan satu transmit hanya dihitung sukses satu kali
    if {[info exists transmitted_this_round($src)] && $transmitted_this_round($src) == 1} {
        incr success_transmit($src)
        set transmitted_this_round($src) 0
    }
...
# deklarasi global untuk flag transmisi pada bagian atas file
global transmitted_this_round

# Perbaikan prosedur utama Q-Learning
proc QLearningNodeMode {} {
    global ns val status actions energy n Q learning_rate prev_energy transmit_success total_transmit transmitted_this_round
    global epsilon epsilon_decay epsilon_min

    # Reset success flag dan flag transmisi sebelum loop
    for {set i 0} {$i < $val(nn)} {incr i} {
        set transmit_success($i) 0
        set transmitted_this_round($i) 0
    }
    ...
    # Penurunan epsilon tiap 10 detik, supaya tetap ada eksplorasi
    if { [expr int([$ns now]) % 10] == 0 && $epsilon > $epsilon_min } {
        set epsilon [expr {$epsilon - $epsilon_decay}]
        if {$epsilon < $epsilon_min} {
            set epsilon $epsilon_min
        }
        puts "DEBUG: Epsilon sekarang: $epsilon pada waktu [$ns now]"
    }
...
proc finish {} {
      global ns tracefile namfile ltf_auto val total_transmit success_transmit srfile
   ...
   # Tutup file lifetime otomatis
    if {[info exists ltf_auto]} {
        close $ltf_auto
    }

     # === Output RL Success Rate ke file ===
    for {set i 0} {$i < $val(nn)} {incr i} {
        set total [expr {[info exists total_transmit($i)] ? $total_transmit($i) : 0}]
        set succ  [expr {[info exists success_transmit($i)] ? $success_transmit($i) : 0}]
        set rate  [expr {$total > 0 ? double($succ)/$total : 0.0}]
        puts $srfile "$i $succ $total $rate"
    }
      close $srfile
